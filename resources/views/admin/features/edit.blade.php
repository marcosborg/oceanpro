                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.features.update", [$feature->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('text') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="text" id="text">{{ old('text', $feature->text) }}</textarea>
                            @if($errors->has('text'))
                        ...
                        <div class="form-group {{ $errors->has('link') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="link" id="link" value="{{ old('link', $feature->link) }}">
                            @if($errors->has('link'))
                        ...
                        <div class="form-group {{ $errors->has('image') ? 'has-error' : '' }}">
                            <label for="image">{{ trans('cruds.feature.fields.image') }}</label>
                            <div class="needsclick dropzone" id="image-dropzone">
                            </div>
                            @if($errors->has('image'))
                                <span class="help-block" role="alert">{{ $errors->first('image') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.feature.fields.image_helper') }}</span>
                        </div>
...
<script>
    ...
    url: '{{ route('admin.features.storeMedia') }}',
    maxFilesize: 2, // MB
...
@if(isset($feature) && $feature->image)
      var file = {!! json_encode($feature->image) !!}
